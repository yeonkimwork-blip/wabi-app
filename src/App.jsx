      ...
      <div className="min-h-screen bg-gray-50">
        <div className="bg-indigo-600 px-6 py-8 text-white">
          ...
          <div>
            ...
            <p className="text-indigo-200 text-sm">{formatDate(currentReflection?.date)} • {currentReflection?.time}</p>
          </div>
        </div>
                  ...
                  {emotions.map((emotion) => (
                    <button
                      ...
                      className={`p-4 border-2 rounded-lg text-center transition-all ${
                        selectedEmotion?.emoji === emotion.emoji ? emotion.selected : `${emotion.bg} ${emotion.border}`
                      }`}
                    >
                  ...
                  {aiMessages.map((message, index) => (
                    <div key={index} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[85%] px-4 py-3 rounded-lg ${
                        message.type === 'ai' ? 'bg-indigo-50 border border-indigo-200' : 'bg-gray-100'
                      }`}>
                        {message.type === 'ai' && (
      ...
      <div className="bg-white px-6 py-8">
        ...
        <div className="space-y-4 mb-8">
          {events.map((event) => {
            ...
            return (
              <div key={event.id} className={`bg-white border rounded-lg p-4 ${isCompleted ? 'opacity-50' : 'border-gray-200'}`}>
  ...
  const AddEventScreen = () => {
    ...
    const handleSubmit = () => {
      ...
      const newEvent = {
        ...
        goal: formData.goal,
        typeColor: formData.type === 'Social' ? 'bg-indigo-100 text-indigo-600' : 
                   formData.type === 'Professional' ? 'bg-amber-100 text-amber-600' : 
