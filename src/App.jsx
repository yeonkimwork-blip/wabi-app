...
function App() {
  ...
  const ReflectionScreen = () => {
    ...
    const generateAIResponse = async (reflection, emotion, conversationHistory = []) => {
      ...
      const readingTime = conversationHistory.length > 0 ? conversationHistory[conversationHistory.length - 1].content.length * 20 : reflection.length * 20;
      ...
      try {
        ...
        const messages = conversationHistory.length > 0 ? conversationHistory : [{ role: "user", content: systemPrompt }];
